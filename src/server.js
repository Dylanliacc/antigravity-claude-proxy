...
function parseError(error) {
  ...
  if (
    ...
    const resetMatch = error.message.match(
      /quota will reset after (\d+h\d+m\d+s|\d+m\d+s|\d+s)/i,
    );
    ...
    if (format === "table") {
      ...
      for (const acc of status.accounts) {
        ...
        if (acc.isInvalid) {
          ...
          const remaining = acc.rateLimitResetTime
            ? acc.rateLimitResetTime - Date.now()
            : 0;
          accStatus =
            remaining > 0
              ? `limited (${formatDuration(remaining)})`
              : "rate-limited";
        } else {
      ...
      const modelColWidth =
        Math.max(25, ...sortedModels.map((m) => m.length)) + 2;
...
app.post("/v1/messages", async (req, res) => {
  try {
    ...
    if (accountManager.isAllRateLimited()) {
      console.log(
        "[Server] All accounts rate-limited. Resetting state for optimistic retry.",
      );
    ...
    // Strip 'claude-' prefix from Gemini models so the backend recognizes them
    let targetModel = model;
    if (targetModel && targetModel.startsWith("claude-gemini-")) {
      targetModel = targetModel.replace("claude-", "");
      console.log(`[API] Mapped alias ${model} to ${targetModel}`);
    }
    ...
    console.log(
      `[API] Request for model: ${request.model}, stream: ${!!stream}`,
    );
 ...
 * Allows projects like DeepWiki to use Antigravity models via OpenAI API format
 */
app.post("/v1/chat/completions", async (req, res) => {
  try {
    ...
    console.log(
      `[API] OpenAI request for model: ${model}, stream: ${!!stream}`,
    );
    ...
    if (stream) {
      ...
      try {
        for await (const event of sendMessageStream(
          ...
          const openaiEvent = convertAnthropicSSEToOpenAI(
            event,
            model,
            messageId,
          );
        ...
        res.write(
          `data: ${JSON.stringify({ error: { message: streamError.message } })}\n\n`,
        );
        res.end();
      }
    } else {
