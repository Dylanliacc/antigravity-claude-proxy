...
async function addAccount(existingAccounts) {
  ...
  try {
    // Race between server and manual input
    const codePromise = startCallbackServer(state);

    console.log(
      "Using remote environment? You can also paste the authorization code below.",
    );
    console.log("1. Complete login in browser.");
    console.log(
      '2. When redirected to localhost, copy the "code" parameter from the URL.',
    );
    console.log("3. Paste it here (or wait for automatic capture):\n");
