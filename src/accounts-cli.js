...
async function verifyAccounts() {
  ...
  for (const account of accounts) {
    try {
      ...
      // If account was marked invalid but now works, repair it
      if (account.isInvalid) {
        console.log(`  ✓ ${email} - RECOVERED (was offline)`);
      ...
      // Optional: could mark as invalid here if strictly desired,
      // but usually we leave that to the separate runtime logic or user decision.
    }
  }

  if (hasChanges) {
    saveAccounts(accounts);
    console.log("\n✓ Successfully recovered offline accounts.");
  }
}
