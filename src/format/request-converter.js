...
export function convertAnthropicToGoogle(anthropicRequest) {
  ...
  if (isThinking) {
    if (isClaudeModel) {
      ...
      if (thinkingBudget) {
        ...
        console.log(
          "[RequestConverter] Claude thinking enabled with budget:",
          thinkingBudget,
        );
      } else {
        console.log(
      ...
      console.log(
        "[RequestConverter] Gemini thinking enabled with budget:",
  ...
  if (tools && tools.length > 0) {
    ...
    console.log(
      "[RequestConverter] Tools:",
      JSON.stringify(googleRequest.tools).substring(0, 300),
    );
  }
