...
/**
 * Get user email from access token (helper)
 */
...
export class AccountManager {
  ...
  async initialize() {
    ...
    try {
      ...
      console.log(
        `[AccountManager] Loaded ${this.#accounts.length} account(s) from config`,
      );
  ...
  async #loadDefaultAccount() {
    try {
      ...
      if (authData?.apiKey) {
        ...
        console.log(
          `[AccountManager] Loaded default account: ${this.#accounts[0].email}`,
        );
      }
    } catch (error) {
      console.error(
  ...
  clearExpiredLimits() {
    ...
    for (const account of this.#accounts) {
      if (
        ...
        cleared++;
        console.log(
          `[AccountManager] Rate limit expired for: ${account.email}`,
        );
      }
    }
  ...
  pickStickyAccount() {
    ...
    if (waitInfo.shouldWait) {
      console.log(
        `[AccountManager] Waiting ${formatDuration(waitInfo.waitMs)} for sticky account: ${waitInfo.account.email}`,
      );
  ...
  markRateLimited(email, resetMs = null) {
    ...
    const cooldownMs =
      resetMs || this.#settings.cooldownDurationMs || DEFAULT_COOLDOWN_MS;
  ...
  markInvalid(email, reason = "Unknown error") {
    ...
    console.log(`[AccountManager] ⚠ Account INVALID: ${email}`);
  ...
  async getTokenForAccount(account) {
    ...
    if (account.source === "oauth" && account.refreshToken) {
      ...
      try {
        ...
        console.log(
          `[AccountManager] Refreshed OAuth token for: ${account.email}`,
        );
      } catch (error) {
        console.error(
  ...
  async #discoverProject(token) {
    for (const endpoint of ANTIGRAVITY_ENDPOINT_FALLBACKS) {
      try {
        ...
        console.log(
          `[AccountManager] Project discovery failed at ${endpoint}:`,
          error.message,
        );
      }
    }
    ...
    console.log(
      `[AccountManager] Using default project: ${DEFAULT_PROJECT_ID}`,
    );
  ...
  /**
   * Attempt to recover offline (invalid) accounts by verifying their tokens
   * @returns {Promise<{recovered: Array, failed: Array}>} Result of recovery attempt
   */
  async recoverOfflineAccounts() {
    ...
    console.log(
      `[AccountManager] Attempting to recover ${invalidAccounts.length} offline accounts...`,
    );

    for (const account of invalidAccounts) {
      try {
        // Determine token source and attempt refresh
        let token;
        if (account.source === "oauth" && account.refreshToken) {
          const tokens = await refreshAccessToken(account.refreshToken);
          token = tokens.accessToken;
          ...
          // Database accounts might not be easily recoverable without external tool interaction,
          // but we can try if there's a stored token?
          // For now, let's skip non-refreshable types or treat as failed if no clear way.
          // Actually, getTokenForAccount logic handles DB extraction, but here we want to verify.
          // Let's assume mostly OAuth for "offline" issues described by user.
          throw new Error("Cannot auto-recover this account type");
        }

        // Verify the token works by getting email
        const email = await getUserEmail(token);

        if (email.toLowerCase() === account.email.toLowerCase()) {
          // Success! Repair the account
          account.isInvalid = false;
          account.invalidReason = null;
          account.lastUsed = Date.now(); // Mark as active
          results.recovered.push(account.email);
          console.log(`[AccountManager] ✓ Recovered account: ${account.email}`);
        } else {
          throw new Error("Email mismatch after refresh");
        }
      } catch (error) {
        console.error(
          `[AccountManager] ✗ Failed to recover ${account.email}:`,
          error.message,
        );
        results.failed.push({
          email: account.email,
